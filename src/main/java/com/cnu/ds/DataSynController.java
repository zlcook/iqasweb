...
public class DataSynController {
	...
	public ModelAndView receive(Model model,HttpServletRequest request){
         ...
         loger.info(sdf.format(new Date())+":来自"+ip+" 同步数据开始");
	  ...
	  try {
			...
			e.printStackTrace();
			loger.error("同步数据出错："+e.getMessage());
		}
		...
		//解析完数据，开始存入数据库
		 if(parseflage){
			  ...
			  {	
			    ...
			    loger.error(sdf.format(new Date())+":来自"+ip+" 同步数据失败：SYN_STUDY_RECORD_FAILURE");
			  }
		       loger.info(sdf.format(new Date())+":来自"+ip+" 同步数据结束");
		 }else{
			 status.setStatus(StatusConstant.SYN_STUDY_RECORD_PARSE_FAILURE);
		     loger.error(sdf.format(new Date())+":来自"+ip+" 同步数据失败：SYN_STUDY_RECORD_PARSE_FAILURE");
		 }
